...
def calcular_importe_total(acreditaciones:dict):
    importe_total = sum(
            (
                Decimal(str(acreditacion.get("importe_acreditado") or "0"))
...
def crearDocumento(
    acreditaciones: list[Acreditaciones], motivos_config: Mapping[str, bool] | None = None
):
    ...
    importe_total = calcular_importe_total(acreditaciones)

    filas = [preparar_fila_baja(acreditacion, motivos_config) for acreditacion in acreditaciones]
